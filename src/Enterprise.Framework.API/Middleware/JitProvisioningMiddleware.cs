...
_next = next;
 ...
 async Task InvokeAsync(HttpContext context, ICurrentUserService currentUserService, IApplicationDbContext dbContext) {
    
if (currentUserService.IsAuthenticated && !string.IsNullOrEmpty(currentUserService.UserId)) {
        
var userId = currentUserService.UserId;

            var cacheKey = $"jit_check_
userId}
";
...
;
                        ...
                        _logger.LogInformation("JIT Provisioning: User 
UserId}
 created successfully.", userId);

                    }

                    catch (DbUpdateException ex) {
                    
_logger.LogWarning(ex, "JIT Provisioning: Conflict detected for user 
UserId}
. Assuming already created by parallel request.", userId);

                    }

                    catch (Exception ex) {
                    
_logger.LogError(ex, "JIT Provisioning: Unexpected error for user 
UserId}
.", userId);
